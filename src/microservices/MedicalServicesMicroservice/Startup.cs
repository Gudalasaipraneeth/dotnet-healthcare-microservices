...
namespace MedicalServicesMicroservice;
...
public class Startup(IConfiguration configuration)
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        services.AddSingleton<IMedicalServicesRepository>(sp =>
          new MedicalServicesRepository(sp.GetService<IMongoDatabase>() ?? throw new Exception("IMongoDatabase not found"))
        );

        services.AddSwaggerGen(c =>
        {
            c.SwaggerDoc("v1", new OpenApiInfo { Title = "Medical Services", Version = "v1" });
